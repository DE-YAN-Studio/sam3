...
if _TRITON_AVAILABLE:
    ...
    def edt_triton(data: torch.Tensor):
        """
        Computes the Euclidean Distance Transform (EDT) using the Triton GPU kernel.
        data: (B, H, W) bool/float tensor on CUDA.
        """
        assert data.dim() == 3
        assert data.is_cuda
        B, H, W = data.shape
    ...
    # Triton is not available on Windows — fall back to scipy for EDT.
    # This is only called during video tracking (fill_holes_in_mask_scores),
    # not during image inference.
    import numpy as np
    from scipy.ndimage import distance_transform_edt as _scipy_edt

    def edt_triton(data: torch.Tensor):
        """
        CPU fallback for edt_triton using scipy.ndimage.distance_transform_edt.
        Functionally equivalent; slower but correct on platforms without Triton.
        data: (B, H, W) bool/float tensor (CUDA or CPU).
        """
        assert data.dim() == 3
        device = data.device
        data_np = data.bool().cpu().numpy()  # (B, H, W)
        out = np.stack([_scipy_edt(b) for b in data_np], axis=0).astype(np.float32)
